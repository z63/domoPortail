...
class pPie
{
	...
	function draw2DPie($X,$Y,$Format="")
	{
		...
		if (count($Values)==1) {
			$WastedAngular = 0;
		} else { $WastedAngular = count($Values) * $DataGapAngle;
		}
		...
		foreach($Values as $Key => $Value)
		{
			...
			else
			{
				if ( !isset($Palette[$ID]["R"]) ) {
					$Color = $this->pChartObject->getRandomColor(); $Palette[$ID] = $Color; $this->pDataObject->savePalette($ID,$Color);
				}
			...
			$Plots = "";
			$EndAngle = $Offset+($Value*$ScaleFactor); if ( $EndAngle > 360 ) {
				$EndAngle = 360;
			}
		...
		if ( $WriteValues != NULL && !$Shadow )
		{
			...
			foreach($Values as $Key => $Value)
			{
				$EndAngle = ($Value*$ScaleFactor) + $Offset; if ( $EndAngle > 360 ) {
					$EndAngle = 0;
				}
				$Angle    = ($EndAngle - $Offset)/2 + $Offset;
	...
	function draw3DPie($X,$Y,$Format="")
	{
		...
		foreach($Values as $Key => $Value)
		{
			...
			$StartAngle = $Offset;
			$EndAngle   = $Offset-($Value*$ScaleFactor); if ( $EndAngle < 0 ) {
				$EndAngle = 0;
			}

			if ( $StartAngle > 180 ) {
				$Visible[$Slice]["Start"] = TRUE;
			} else { $Visible[$Slice]["Start"] = TRUE;
			}
			if ( $EndAngle < 180 )   {
				$Visible[$Slice]["End"] = FALSE;
			} else { $Visible[$Slice]["End"] = TRUE;
			}
	...
	function shift($StartAngle,$EndAngle,$Offset,$Reversed)
	{
		...
		foreach($this->LabelPos as $Key => $Settings)
		{
			if ( $Settings["Angle"] > $StartAngle && $Settings["Angle"] <= $EndAngle ) {
				$this->LabelPos[$Key]["YTop"] = $Settings["YTop"] + $Offset; $this->LabelPos[$Key]["YBottom"] = $Settings["YBottom"] + $Offset; $this->LabelPos[$Key]["Y2"] = $Settings["Y2"] + $Offset;
			}
		}
	}
		...
		foreach($Values as $Key => $Value)
		{
			...
			for($i=$Offset;$i<=$EndAngle;$i=$i+$Step)
			{
				...
				if ( !isset($Boundaries[0]["X1"]) ) {
					$Boundaries[0]["X1"] = $Xc; $Boundaries[0]["Y1"] = $Yc;
				}
			...
			for($i=$EndAngle;$i>=$Offset;$i=$i-$Step)
			{
				...
				if ( !isset($Boundaries[1]["X2"]) ) {
					$Boundaries[1]["X2"] = $Xc; $Boundaries[1]["Y2"] = $Yc;
				}
				$AAPixels[] = array($Xc,$Yc);
	...
	function arraySerialize($Data)
	{
		...
		foreach($Data as $Key => $Value)
		{
			if ($Result == "") {
				$Result = floor($Value);
			} else { $Result = $Result.",".floor($Value);
			}
		}
