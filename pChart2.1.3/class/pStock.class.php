...
class pStock
{
	...
	function drawStockChart($Format="")
	{
		...
		if ( $BoxUpSurrounding != NULL )	{
			$BoxUpBorderR = $BoxUpR + $BoxUpSurrounding; $BoxUpBorderG = $BoxUpG + $BoxUpSurrounding; $BoxUpBorderB = $BoxUpB + $BoxUpSurrounding;
		}
		if ( $BoxDownSurrounding != NULL )	{
			$BoxDownBorderR = $BoxDownR + $BoxDownSurrounding; $BoxDownBorderG = $BoxDownG + $BoxDownSurrounding; $BoxDownBorderB = $BoxDownB + $BoxDownSurrounding;
		}
		...
		foreach($Plots as $Key =>$Points)
		{
			...
			} else { $ImageMapColor = $this->pChartObject->toHTMLColor($BoxDownR,$BoxDownG,$BoxDownB);
			}
			...
			elseif ( $Data["Orientation"] == SCALE_POS_TOPBOTTOM )
			{
				...
				if ( $XDivs == 0 ) {
					$XStep = 0;
				} else { $XStep = ($this->pChartObject->GraphAreaY2-$this->pChartObject->GraphAreaY1-$XMargin*2)/$XDivs;
				}
