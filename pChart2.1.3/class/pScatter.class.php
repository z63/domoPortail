...
class pScatter
{
			...
			if ( $AxisSettings["Identity"] == AXIS_X )
			{
				if ( $AxisSettings["Position"] == AXIS_POSITION_BOTTOM )
				{
					if ( $XLabelsRotation == 0 )				   {
						$LabelAlign = TEXT_ALIGN_TOPMIDDLE; $LabelOffset = 2;
					}
					if ( $XLabelsRotation > 0 && $XLabelsRotation < 190 )   {
						$LabelAlign = TEXT_ALIGN_MIDDLERIGHT; $LabelOffset = 5;
					}
					if ( $XLabelsRotation == 180 )			   {
						$LabelAlign = TEXT_ALIGN_BOTTOMMIDDLE; $LabelOffset = 5;
					}
					if ( $XLabelsRotation > 180 && $XLabelsRotation < 360 ) {
						$LabelAlign = TEXT_ALIGN_MIDDLELEFT; $LabelOffset = 2;
					}
				...
				{
					if ( $XLabelsRotation == 0 )					{
						$LabelAlign = TEXT_ALIGN_BOTTOMMIDDLE; $LabelOffset = 2;
					}
					if ( $XLabelsRotation > 0 && $XLabelsRotation < 190 )	{
						$LabelAlign = TEXT_ALIGN_MIDDLELEFT; $LabelOffset = 2;
					}
					if ( $XLabelsRotation == 180 )				{
						$LabelAlign = TEXT_ALIGN_TOPMIDDLE; $LabelOffset = 5;
					}
	...
	function drawScatterPlotChart($Format=NULL)
	{
		...
		foreach($Data["ScatterSeries"] as $Key => $Series)
		{
			if ( $Series["isDrawable"] == TRUE )
			{
				...
				} else { $Description = $ImageMapTitle;
				}
				...
				if ( !is_array($PosArrayX) ) {
					$Value = $PosArrayX; $PosArrayX = ""; $PosArrayX[0] = $Value;
				}
				$PosArrayY = $this->getPosArray($SerieValuesY,$SerieYAxis);
				if ( !is_array($PosArrayY) ) {
					$Value = $PosArrayY; $PosArrayY = ""; $PosArrayY[0] = $Value;
				}
			...
			if ( $Series["isDrawable"] == TRUE )
			{
				...
				else
				{
					...
					if ( $Y1 < $this->pChartObject->GraphAreaY1 ) {
						$X1 = $X1 + ($this->pChartObject->GraphAreaY1-$Y1/$RealM); $Y1 = $this->pChartObject->GraphAreaY1;
					}
					if ( $Y1 > $this->pChartObject->GraphAreaY2 ) {
						$X1 = $X1 + ($Y1-$this->pChartObject->GraphAreaY2)/$RealM; $Y1 = $this->pChartObject->GraphAreaY2;
					}
					if ( $Y2 < $this->pChartObject->GraphAreaY1 ) {
						$X2 = $X2 - ($this->pChartObject->GraphAreaY1-$Y2)/$RealM; $Y2 = $this->pChartObject->GraphAreaY1;
					}
					if ( $Y2 > $this->pChartObject->GraphAreaY2 ) {
						$X2 = $X2 - ($Y2-$this->pChartObject->GraphAreaY2)/$RealM; $Y2 = $this->pChartObject->GraphAreaY2;
					}
				}
	...
	function writeScatterLabel($ScatterSerieID,$Points,$Format="")
	{
		...
		if ( !is_array($Points) ) {
			$Point = $Points; $Points = ""; $Points[0] = $Point;
		}
		...
		foreach($Points as $Key => $Point)
		{
			if ( isset($PosArrayX[$Point]) && isset($PosArrayY[$Point]) )
			{
				...
				} else { $XValue = round($SerieValuesX[$Point],$Decimals);
				}
				...
				} else { $YValue = round($SerieValuesY[$Point],$Decimals);
				}
