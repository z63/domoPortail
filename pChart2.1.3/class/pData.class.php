...
class pData
{
	...
	function addPoints($Values,$SerieName="Serie1")
	{
		...
		if ( $Values != VOID )
		{
			...
			if ( empty($StrippedData) ) {
				$this->Data["Series"][$SerieName]["Max"] = 0; $this->Data["Series"][$SerieName]["Min"] =0; return(0);
			}
	...
	function stripVOID($Values)
	{
		if (!is_array($Values)) {
			return(array());
		} $Result = array(); foreach($Values as $Key => $Value) {
			if ( $Value != VOID ) {
				$Result[] = $Value;
			}
		} return($Result);
	...
	function setScatterSerie($SerieX,$SerieY,$ID=0)
	{
		if (isset($this->Data["Series"][$SerieX]) && isset($this->Data["Series"][$SerieY]) ) {
			$this->initScatterSerie($ID); $this->Data["ScatterSeries"][$ID]["X"] = $SerieX; $this->Data["ScatterSeries"][$ID]["Y"] = $SerieY;
		}
	}
	...
	function getGeometricMean($Serie)
	{
		...
		{
			$SerieData = $this->stripVOID($this->Data["Series"][$Serie]["Data"]);
			$Seriesum  = 1; foreach($SerieData as $Key => $Value) {
	...
	function getHarmonicMean($Serie)
	{
		...
		{
			$SerieData = $this->stripVOID($this->Data["Series"][$Serie]["Data"]);
			$Seriesum  = 0; foreach($SerieData as $Key => $Value) {
	...
	function createFunctionSerie($SerieName,$Formula="",$Options="")
	{
		...
		for($i=$MinX; $i<=$MaxX; $i=$i+$XStep)
		{
			...
			if ( $return == "!" ) {
				$return = VOID;
			} else { $return = $this->right($return,strlen($return)-1);
			}
			if ( $return == "NAN" ) {
				$return = VOID;
			}
